...
(function () {
  ...
  function initSlider(root) {
    ...
    new Swiper(container, {
      ...
      breakpoints: {
        ...
        900: {
          slidesPerView: isServicesOr ? 2 : isOr ? 3 : 2.5,
          spaceBetween: isServicesOr ? 140 : isOr ? 140 : 24
        }
      }
    });
  }
  ...
  function tryInit(count = 0) {
    if (typeof window.Swiper !== "function") {
      if (count < MAX_TRIES) setTimeout(() => tryInit(count + 1), INTERVAL);
      return;
    }
    SLIDERS.forEach(sel => {
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll(".wrap-sections").forEach(trigger => {
      ...
      if (border) wrapper.appendChild(border);
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    try {
      ...
      if (sections.length) {
        sections.forEach(section => menuDM.appendChild(section.cloneNode(true)));
    ...
    function applyMenuPadding() {
      ...
      if (contentWrapper) contentWrapper.style.paddingTop = `${height}px`;
    }
  ...
  // =============================================
  // 5. HEADER SHRINK — NAV ITEMS VISIBILITY
  // =============================================
  document.addEventListener("DOMContentLoaded", () => {
    const header = document.querySelector("#header");
  ...
  // =============================================
  // 6. PORTFOLIO PAGE (hover + video + HLS + mobile 2-tap)
  // =============================================
  (function () {
    ...
    function normalizeCategory(str) {
      return (str || '').toLowerCase().trim()
        .replace(/^category:\s*/i, '').replace(/&/g, 'and').replace(/\s+/g, '-');
    }
    ...
    function parseCategoriesFromText(text) {
      ...
      if (!cleaned) return [];
      return cleaned.split(',').map(item => normalizeCategory(item)).filter(Boolean);
    }
    ...
    function pickFirstMeaningfulText(root) {
      if (!root) return '';
      for (const el of root.querySelectorAll('p, h1, h2, h3, h4, h5, h6, span')) {
    ...
    async function resolvePortfolioSourceFromItemPage(fullUrl) {
      ...
      if (sourceSection) {
        ...
        if (txt) linkText = txt;
        sourceSection.querySelectorAll('.markdown-block').forEach(block => {
          const txt = pickFirstMeaningfulText(block);
          if (/^category:/i.test(txt || '')) categories = parseCategoriesFromText(txt);
        });
      }
    ...
    async function ensureBgMedia(bgItem, titleText) {
      ...
      if (videoType === 'external') {
        ...
        if (!raw) return;
        let src = '';
        if (isYouTube(raw)) { src = youtubeEmbed(raw) || raw; }
        else if (isVimeo(raw)) { src = vimeoEmbed(raw) || raw; }
    ...
    async function setActiveNumber(n, bgsWrap) {
      ...
      const tEl = titlesWrap.querySelector(`.portfolio-cust-title[data-item-number="${n}"]`);
      const lEl = linksWrap.querySelector(`.portfolio-cust-link[data-item-number="${n}"]`);
      const b   = bgsWrap.querySelector(`.portfolio-cust-bgItem[data-item-number="${n}"]`);

      if (tEl) tEl.classList.add('is-active');
      if (lEl) lEl.classList.add('is-active');
      if (!b) return;
      ...
      try {
        ...
        if (activeCategory) {
          ...
          async function filterWorker() {
            while (filterCursor < all.length) {
              ...
              if (info && Array.isArray(info.categories) && info.categories.includes(activeCategory)) {
                matched.push(it);
              }
            }
          }
          const filterWorkers = [];
          for (let i = 0; i < CONCURRENCY_FILTER; i++) filterWorkers.push(filterWorker());
        ...
        async function worker() {
          while (cursor < itemsToRender.length) {
            ...
            aTitle.href = href; aTitle.textContent = title;
            ...
            if (imgUrl) {
              ...
              if (idx < 2) { img.loading = 'eager'; try { img.fetchPriority = 'high'; } catch (e) {} }
        ...
        function getNFromEl(el) { return el ? el.getAttribute('data-item-number') || '' : ''; }
        ...
        if (!isTouch) {
          ...
          function prewarm(n) {
            ...
            prewarmTimer = setTimeout(async () => {
              ...
              if (v) {
                const ds = v.getAttribute('data-src');
                if (ds && !v.getAttribute('src')) { v.setAttribute('src', ds); try { v.load(); } catch (e) {} }
              }
            }, 200);
          }
        ...
        if (isTouch) {
          ...
          function onLinkTapCapture(e) {
            ...
            if (!href) return;
            if (href === armedHref) { disarm(); return; }
  ...
  // =============================================
  // 7. SCROLLING IMAGES — PORTFOLIO ITEM PAGE
  // =============================================
  document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('[id^="scrolling-images"] .gallery-block .slide').forEach(slide => {
      ...
      next.querySelectorAll('.scrolling-images .first, .scrolling-images .second')
        .forEach(t => t.insertAdjacentHTML('beforeend', `<div class="image-wrapper"><img src="${src}"></div>`));
    });
  });


  // =============================================
  // 8. POPUP FORM PAGE (Fancybox)
  // =============================================
  document.addEventListener("DOMContentLoaded", () => {
    ...
    function runSquarespaceAfterBodyLoad() {
      ...
      if (sqs && typeof sqs.afterBodyLoad === "function") {
        try { sqs.AFTER_BODY_LOADED = false; sqs.afterBodyLoad(); }
        catch (e) { console.warn("Squarespace afterBodyLoad() failed", e); }
      }
    }
    ...
    function preventAutoPlay(scope) {
      ...
      scope.querySelectorAll("video").forEach((v) => {
        try { v.autoplay = false; v.removeAttribute("autoplay"); v.pause(); } catch (_) {}
      });
    }
    ...
    function initFAQ() {
      ...
      column.querySelectorAll(".faq-sec").forEach(sec => {
        ...
        trigger.addEventListener("click", () => {
          ...
          if (isOpen) { sec.classList.remove("active"); animate(content, false); }
          else { sec.classList.add("active"); animate(content, true); }
        });
      });
    }

    document.addEventListener("DOMContentLoaded", () => setTimeout(initFAQ, 300));
    window.addEventListener("popstate", () => setTimeout(initFAQ, 300));
  })();


  // =============================================
  // 10. ARROW DOWN — SCROLL TO NEXT SECTION
  // =============================================
  (function () {
    const nextSection = s => {
  ...
  // =============================================
  // 11. RESOURCES PAGE — HOVER BLOCK COLORS
  // =============================================
  document.addEventListener("DOMContentLoaded", function () {
    ...
    container.querySelectorAll(".list-item").forEach(item => {
      ...
      if (!text) return;
      let color = null;
      if (/^#([0-9a-f]{3,4}|[0-9a-f]{6}|[0-9a-f]{8})$/i.test(text)) { color = text; }
      else if (/^rgba?\(/i.test(text)) { color = text; }
      if (color) item.style.background = color;
    });
  });
  ...
  // =============================================
  // 13. CART TEXT FIX ("Added!" → "Added")
  // =============================================
  document.addEventListener("DOMContentLoaded", function () {
    function fixTexts() {
      document.querySelectorAll('.cart-added-text').forEach(el => {
        if (el.textContent.trim() === 'Added!') el.textContent = 'Added';
      });
      document.querySelectorAll('.commerce-mini-cart-header-title .fs-unmask').forEach(el => {
        if (el.textContent.trim() === 'Added to cart!') el.textContent = 'Added to cart';
      });
    }
    fixTexts();
    new MutationObserver(() => fixTexts()).observe(document.body, {
  ...
  // =============================================
  // 14. ACTIVE NAV LINK HIGHLIGHTER
  // =============================================
  document.addEventListener("DOMContentLoaded", function () {
    try {
      ...
      navItems.forEach(function (item) {
        ...
        folderLinks.forEach(link => link.classList.remove("active-sublink"));

        let isActive = false;

        if (directLink) {
          const linkKey = getUrlKey(directLink.getAttribute("href"));
          if (linkKey === currentKey) isActive = true;
        }

        if (!isActive && folderBtn) {
          const folderKey = getUrlKey(folderBtn.getAttribute("data-href"));
          if (folderKey === currentKey) isActive = true;
        }
        ...
        if (folderLinks.length) {
          folderLinks.forEach(function (link) {
            ...
            if (
              subKey === currentKey ||
              (subKey && !subKey.includes("?") && subPath !== "/" && currentPath.startsWith(subPath + "/"))
            ) {
        ...
        if (isActive) item.classList.add("active-link");
      });
